...
fn parse_request<P: Read + Write>(conn: &mut P, config: Config) {
            ...
            Some(vec) => {
                ...
                for c in vec.iter() {
                    if c.0 == stat_line[1] {
                        ...
                        res = (line, c.1(request), "text/plain");
    ...
    log::debug!(
        "{:#?}",
        get_header(headers.clone(), "Accept-Encoding".to_string())
    );

    let compression =
        get_header(headers.clone(), "Accept-Encoding".to_string()).unwrap_or(Vec::new());

    #[cfg(feature = "sys")]
    if compression.contains(&"br".to_string()) || compression.contains(&"gzip".to_string()) {
        if config.get_gzip() {
            #[cfg(feature = "log")]
